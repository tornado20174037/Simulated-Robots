...
class Robot:
    ...
    def obstacle_avoidance(self, min_safe_distance=2.5):
...
def run_demo():
    ...
    while not rospy.is_shutdown():
        ...
        if robot.obstacle_avoidance():
            ...
            robot.set_speed_angle(0, math.radians(avoidance_angle))
            rospy.sleep(1)
            robot.set_speed_angle(1, 0)# Keep moving with this direction
            rospy.sleep(3)
            robot.set_speed_angle(0, -math.radians(avoidance_angle+5))
            rospy.sleep(1)
            robot.set_speed_angle(1, 0)# Keep moving with this direction
            
            #robot.set_speed_angle(0.5, 0.3)
            #rospy.sleep(2)  
